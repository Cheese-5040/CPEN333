...
import time
from turtle import left, right   #is used to cause some delay to simulate thinking or eating times
...
def philosopher(id: int, chopstick: list): 
    ...
    for _ in range(6): #to make testing easier, instead of a forever loop we use a finite loop
        ...
        with chopstick[leftChopstick]:
            print(f"DEBUG: philosopher{id} has chopstick{leftChopstick}")
            locked = chopstick[rightChopstick].acquire(block=False) #do not block the lock since we just testing it, and see if we can acquire it or not 
            # print(f"check right chopstick condition, {locked}")
            # if can succesfully pick up right chopstick, then 
            if locked: #if right chopstick is free to use = true
                chopstick[rightChopstick].release() #release the falsely acquired block from myself 
                with chopstick[rightChopstick]:
            ...
            else : #cannot obtain the right chopstick because it is in use
                print(f"DEBUG: philosopher{id} is to release chopstick {leftChopstick}")
